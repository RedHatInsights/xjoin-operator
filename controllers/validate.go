...
func (i *ReconcileIteration) validate() (isValid bool, mismatchRatio float64, mismatchCount int64, hostCount int64, err error) {
	...
	esCount, err := i.ESClient.CountIndex(elasticsearch.ESIndexName(i.Instance.Status.PipelineVersion))
	if err != nil {
		return false, -1, -1, -1, err
	}
	...
	esIds, err := i.ESClient.GetHostIDs(elasticsearch.ESIndexName(i.Instance.Status.PipelineVersion))
	if err != nil {
		return false, -1, -1, -1, err
	}
