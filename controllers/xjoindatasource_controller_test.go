...
func newXJoinDataSourceReconciler(namespace string) *XJoinDataSourceReconciler {
...
func checkError(err error) {
	if err != nil {
		testLogger.Error(err, "test failure")
	}
	Expect(err).ToNot(HaveOccurred())
}

func createValidDataSource(name string) {
...
func reconcileDataSource(name string) v1alpha1.XJoinDataSource {
	ctx := context.Background()
